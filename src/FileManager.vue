<template>
    <div class="panel-file-manager fm" v-bind:class="{ 'fm-full-screen': fullScreen }">
        <!-- <folder-tree /> -->
        <!-- <navbar /> -->
        <!-- <notification /> -->
        <!-- <context-menu /> -->
        ...
        <template>
            ...
            <folder-tree />
            <!-- header -->
            <navbar manager="left" :settings="settings" />
            <!-- body -->
            <div class="fm-body">
        ...
        <!-- <template v-else-if="windowsConfig === 3">
              <left-manager class="col-12 col-sm-6" manager="left" v-on:click.native="selectManager('left')" v-on:contextmenu.native="selectManager('left')"> </left-manager>
              <right-manager class="col-12 col-sm-6" manager="right" v-on:click.native="selectManager('right')" v-on:contextmenu.native="selectManager('right')"> </right-manager>
          </template> -->
        <!-- -->
        <!-- <info-block /> -->
    </div>
                    ...
                    if (Object.prototype.hasOwnProperty.call(response.data, 'result')) {
                        if (response.data.result.message) {
                            const message = {
                                ...
                                message: Object.prototype.hasOwnProperty.call(this.lang.response, response.data.result.message) ? this.lang.response[response.data.result.message] : response.data.result.message,
                            };
                            // show notification
                ...
                (error) => {
                    ...
                    if (error.response) {
                        ...
                        if (error.response.data.message) {
                            const trMessage = Object.prototype.hasOwnProperty.call(this.lang.response, error.response.data.message) ? this.lang.response[error.response.data.message] : error.response.data.message;
