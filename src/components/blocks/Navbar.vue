	...
	<div class="fm-navbar">
		...
		<div class="fm-navbar_tools_1 border-bottom p-3">
			<div class="btn-group" role="group">
				<button
					type="button"
					class="btn btn-outline-primary btn-tool"
			...
			<div class="btn-group" role="group">
				...
				<button
					type="button"
					class="btn btn-outline-primary btn-tool"
					v-bind:disabled="!isAnyItemSelected || selectedCount > 1 || selectedItem.type == 'dir'"
				...
				<button
					type="button"
					class="btn btn-outline-primary btn-tool"
					v-bind:disabled="!isAnyItemSelected || selectedCount > 1"
					v-on:click="showModal('Rename')"
					v-bind:title="lang.btn.rename"
				>
				...
				<button
					...
					class="btn btn-outline-primary btn-tool"
					v-bind:disabled="!isAnyItemSelected || selectedCount > 1"
					v-on:click="showModal('Properties')"
					v-bind:title="lang.btn.properties"
				>
					<i class="fas fa-eye" />
					<p>Preview</p>
				</button>
			...
			<div class="btn-group" role="group">
				...
				<button
					...
					class="btn btn-outline-primary btn-tool"
					v-bind:disabled="!isAnyItemSelected || selectedCount > 1 || selectedItem.type == 'dir'"
					v-on:click="showModal('GetLink')"
					v-bind:title="lang.btn.getLink"
				>
					<i class="fas fa-link" />
					<p>Copy Link</p>
				</button>
			...
			<div class="btn-group" role="group">
				<button
					type="button"
					class="btn btn-outline-danger btn-tool"
		...
		>
			<div class="row w-100 m-0">
				<div
					class="col-xl-2 p-0 order-last order-xl-first mt-3 mt-xl-0"
				>
					<label
						for="checkAll"
						class="
				...
				<div
					class="col-xl-10 d-flex p-0 pl-xl-2 flex-column flex-md-row"
				>
					<div
						class="
							d-flex
						...
						"
					>
						<label class="form-label m-0 mr-2 tx-medium"
							>Sorting</label
						>
						<v-select
							ref="mySelect"
							:options="option_sortBy"
							:searchable="false"
							:clearable="false"
							v-model="selected_sortBy"
							@input="onSelected()"
							class="vue-select wd-100p h-100 no-search no-clear"
						></v-select>
					</div>
					<div
						class="
					...
					<div class="d-flex align-items-center w-100 wd-md-45p-f">
						...
						<input
							...
							value=""
							name="Search"
							class="form-control d-inline-block bd-gray-200"
							v-model="search"
						/>
						<button
							type="button"
							class="btn btn-outline-primary ml-2 tx-uppercase"
...
<script>
	...
	export default {
		...
		computed: {
			...
			isRootPath() {
				return (
					this.$store.state.fm[this.manager].selectedDirectory === null
				);
			},
			/**
			 * Active manager name
			...
			isAnyItemSelected() {
				return (
					this.$store.state.fm[this.activeManager].selected.files.length >
						0 ||
