<template>
    <div class="fm-footer fm-info-block d-flex align-items-center justify-content-between bg-gray-200 px-3 pd-y-10">
        <div class="tx-medium tx-file-manager-total">
            <span v-show="selectedCount">
                {{ `${selectedCount}` }} of {{ `${directoriesCount + filesCount}` }} selected
        ...
        <button class="btn btn-outline-light IfModal" data-dismiss="modal" aria-label="Close">CANCEL</button>

        <button type="button" class="btn btn-primary IfModal" data-dismiss="modal" aria-label="Close"
  ...
  computed: {
    ...
    isAnyItemSelected() {
      return (
          ...
          this.$store.state.fm[this.activeManager].selected.directories.length != 1
      )
    }
  },
  methods: {
    ...
    selectPic(){
      let path = 'https://d1g0p9cd3qju55.cloudfront.net/' + this.$store.getters[`fm/selectedItems`][0].path;
      ...
      el.closest('.image_upload').find('.description_file_upload').text(reName[reName.length - 1]);
      el.closest('.image_upload').find('.upload-byFileManager, .upload-byDropify').fadeOut();

      $('body').on('click', '.filemanager-clear', function() {
          el.find('.dropify-wrapper').remove();
          el.find('input').val('');
          el.closest('.image_upload').find('.description_file_upload').text('Minimum dimension of image is 1200x202px. Size limit is 2MB. Accepted file types is JPG and PNG.');
          el.closest('.image_upload').find('.upload-byFileManager, .upload-byDropify').fadeIn();
      });
    }
  },
};
